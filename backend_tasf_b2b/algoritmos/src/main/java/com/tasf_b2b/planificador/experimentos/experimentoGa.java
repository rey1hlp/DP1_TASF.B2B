...
public class experimentoGa {
    ...
    private static final long MAX_TIEMPO_MS = 0L;
    private static final int MAX_ESTANCAMIENTO = 0;
    ...
    public static void main(String[] args) {
        ...
        String estancamientoStr = opts.get("estancamiento");
        boolean buscarColapso = "1".equals(opts.getOrDefault("buscar-colapso", "0"));
        ...
        int tamPoblacion = parseEnteroSeguro(opts.getOrDefault("poblacion", String.valueOf(TAM_POBLACION)), TAM_POBLACION);
        int maxGeneraciones = parseEnteroSeguro(opts.getOrDefault("generaciones", String.valueOf(MAX_GENERACIONES)), MAX_GENERACIONES);
        double tasaCruce = parseDoubleSeguro(opts.getOrDefault("cruce", String.valueOf(TASA_CRUCE)), TASA_CRUCE);
        double tasaMutacion = parseDoubleSeguro(opts.getOrDefault("mutacion", String.valueOf(TASA_MUTACION)), TASA_MUTACION);
        int tamTorneo = parseEnteroSeguro(opts.getOrDefault("torneo", String.valueOf(TAMANO_TORNEO)), TAMANO_TORNEO);
        int maxHilos = parseEnteroSeguro(
            opts.getOrDefault("hilos", String.valueOf(Math.max(1, Runtime.getRuntime().availableProcessors() - 1))),
            Math.max(1, Runtime.getRuntime().availableProcessors() - 1)
        );
        boolean paralelo = !"0".equals(opts.getOrDefault("parallel", "1"));
        boolean logGeneraciones = "1".equals(opts.getOrDefault("log-gen", "0"));

        System.out.println("=== EXPERIMENTO NUMERICO - Algoritmo Genetico (GA) ===");
        System.out.printf(
            "Configuracion: %d individuos x %d generaciones x %d replicas | paralelo=%s | hilos=%d",
            tamPoblacion,
            maxGeneraciones,
            replicas,
            paralelo,
        ...
        if (maxEnvios > 0) {
            System.out.print(" | max-envios=" + maxEnvios);
        }
        System.out.println();
        ...
        try {
            ...
            if (buscarColapso) {
                ...
                System.out.printf(
                    Locale.US,
                    "Ventana colapso %s -> %s | envios=%d | maletas=%d | factible=%s%n",
                    UtilArchivos.formatearFecha(diaMin),
                    UtilArchivos.formatearFecha(diaMax),
                    envios.size(),
                    ventana.totalMaletas,
                    ventana.factible
                );
                System.out.printf(
                    "Punto de quiebre aproximado: %s -> %s | envios=%d | maletas=%d | factible=%s%n",
                    UtilArchivos.formatearFecha(diaMin),
                    UtilArchivos.formatearFecha(diaMax),
                    envios.size(),
                    ventana.totalMaletas,
            ...
            System.out.printf(
                "Ventana envios: %s -> %s | dias extra vuelos: %d%n",
                UtilArchivos.formatearFecha(diaMin),
                UtilArchivos.formatearFecha(diaMax),
                Math.max(0, diasExtra)
            );
            System.out.printf(
                "Datos cargados (%s) -> Aeropuertos: %d | Planes: %d | Vuelos: %d | Envios: %d | Maletas: %d%n",
                archivoEnvios,
                aeropuertos.size(),
                planes.size(),
                vuelos.size(),
            ...
            try (PrintWriter csv = new PrintWriter(new FileWriter(rutaSalida.toFile()))) {
                csv.println(
                    "Replica,Semilla,Funcion Objetivo,% Entregados con SLA,Violaciones SLA," +
                ...
                System.out.printf(
                    "| %-4s | %-7s | %-14s | %-7s | %-5s | %-7s | %-5s | %-7s | %-9s | %-8s | %-10s |%n",
                    "Rep",
                    "Semilla",
                    "Fitness",
                    "%SLA",
                ...
                for (int rep = 1; rep <= replicas; rep++) {
                    System.out.println("\n--- Replica GA " + rep + "/" + replicas + " ---");

                    ParametrosGa params = construirParametrosGa(
                        tamPoblacion,
                        maxGeneraciones,
                        tasaCruce,
                        tasaMutacion,
                    ...
                    PlanificadorGa planificador = new PlanificadorGa(grafo, envios, params, rep);
                    ...
                    System.out.printf(
                        Locale.US,
                        "| %-4d | %-7d | %-14.2f | %-7.2f | %-5d | %-7d | %-5d | %-7d | %-9.2f | %-8.2f | %-10d |%n",
                        rep,
                        rep,
                        mejor.fitness,
                        r.pctSla,
                ...
                if (reporteSinRuta && mejorGlobal != null) {
                    Path reporte = ReporteSinRuta.escribirReporte(Paths.get(raiz), envios, mejorGlobal, "GA" + repMejor);
                    if (reporte != null) {
    ...
    private static ResultadoReplica calcularMetricas(
        Individuo mejor,
        List<Envio> envios,
    ...
    private static ParametrosGa construirParametrosGa(
        ...
        int tamTorneo,
        String maxTiempoStr,
        String estancamientoStr,
        boolean paralelo,
        int maxHilos,
        boolean logGeneraciones
    ) {
        ParametrosGa params = new ParametrosGa();
        params.tamanoPoblacion = tamPoblacion;
        params.maxGeneraciones = maxGeneraciones;
        params.tasaCruce = tasaCruce;
        params.tasaMutacion = tasaMutacion;
        params.tamanoTorneo = tamTorneo;
        params.maxTiempoMs = (maxTiempoStr != null && !maxTiempoStr.isBlank())
            ? parseLongSeguro(maxTiempoStr, MAX_TIEMPO_MS)
            : MAX_TIEMPO_MS;
        params.maxGeneracionesSinMejora = (estancamientoStr != null && !estancamientoStr.isBlank())
            ? parseEnteroSeguro(estancamientoStr, MAX_ESTANCAMIENTO)
            : MAX_ESTANCAMIENTO;
        params.evaluacionParalela = paralelo;
        params.maxHilosEvaluacion = maxHilos;
        params.logGeneraciones = logGeneraciones;
    ...
    private static Map<String, String> parseArgs(String[] args) {
        ...
        if (!map.containsKey("envios") && posicionales.size() > 0) {
            map.put("envios", posicionales.get(0));
        }
        if (!map.containsKey("inicio") && posicionales.size() > 1) {
            map.put("inicio", posicionales.get(1));
        }
        if (!map.containsKey("fin") && posicionales.size() > 2) {
            map.put("fin", posicionales.get(2));
        }
    ...
    private static double parseDoubleSeguro(String valor, double def) {
        try {
